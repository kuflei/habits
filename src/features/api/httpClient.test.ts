...
describe("httpClient", () => {
  ...
  it("Should make a GET request and return data", async () => {
    ...
    expect(mockFetch).toHaveBeenLastCalledWith(`${baseUrl}/test-endpoint`, {
      method: "GET",
  ...
  it('Should handle a 404 GET request', async () => {
    ...
    await expect(client.get('/test-endpoint')).rejects.toThrow('Error GET /test-endpoint')
  });
  ...
  it('Should failed POST request', async () => {
    ...
    await expect(client.post('/test-endpoint', payload)).rejects.toThrow('HTTP request failed: Error: Error POST /test-endpoint: 400 - Bad Request');
  });
  ...
  it('should failed DELETE request', async () => {
    ...
    await expect(client.delete('/test-endpoint')).rejects.toThrow('HTTP request failed: Error: Error DELETE /test-endpoint: 400 - Bad Request');
  });

...
import { useAddHabits, useUpdateHabit } from "./hooks/useHabits.ts";
...
const HabitForm: React.FC<HabitFormProps> = (props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (props.habit) {
        updateHabit.mutate([userId, { ...props.habit, ...values }], {
          onError: (error) => {
        ...
        addHabit.mutate([userId, newHabit], {
          onError: (error) => {
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <Typography variant="h5" sx={{ mb: 3 }}>
        {props.habit ? t("editHabit") : t("addHabit")}
      </Typography>
